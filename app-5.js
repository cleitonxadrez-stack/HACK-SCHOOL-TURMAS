...
function erroSupabase(e){
  ...
  if(m.includes('não gerencia este polo')) return 'Você não gerencia este polo.';
  if(m.includes('destino já fechou')) return 'A turma escolhida já fechou. Escolha uma que ainda está formando.';
  if(m.includes('destino está completa')) return 'Essa turma já está completa.';
  if(m.includes('ainda está formando')) return 'A turma ainda está formando — o mentor é escolhido pela equipe quando ela fechar.';
  if(m.includes('limite de turmas')) return 'Esse mentor já está no limite de turmas.';
...
async function carregarSessao(){
  ...
  try{ await sb.rpc('fn_vincular_inscricoes'); }catch(e){ console.warn('vincular', e); }
  const uid = sessao.user.id;

  // allSettled: se uma consulta falhar, as outras seguem e o login não trava
  ...
  const ok = i => r[i].status==='fulfilled' ? r[i].value : {data:null, count:0};
  const perfil=ok(0), mentor=ok(1), coord=ok(2), insc=ok(3);
  r.forEach((x,i)=>{ if(x.status==='rejected') console.warn('sessao['+i+']', x.reason); });
...
async function carregarAdmin(){
  const [insc, turmas, polos, slots, mentores, ms, coords] = await Promise.all([
    ...
    sb.from('polo_coordenadores').select('polo_id,perfil_id,perfis(nome,email)')
  ]);
...
function vAdmin(d){
  ...
  const corpo = adminAba==='cadastros' ? vCadastros(d)
    : adminAba==='clubes' ? vClubes(d)
    : adminAba==='inscricoes' ? `
  ...
  return `<section class="secao" style="border-top:none;padding-top:38px"><div class="molde">
    ...
    <div class="abas">
      ...
      <button class="aba ${adminAba==='clubes'?'on':''}" data-adab="clubes">Clubes</button>
...
/* =====================================================================
   ADMIN · CLUBES (gestão de turma, alunos, mentor e coordenadores)
   ===================================================================== */
let clubeAberto = null;

function vClubes(d){
  const ehAdmin = sessao.perfil?.papel === 'admin';
  const porTurma = {};
  d.insc.forEach(i=>{ (porTurma[i.turma_id] = porTurma[i.turma_id]||[]).push(i); });

  const turmas = d.turmas.slice().sort((a,b)=>
    (a.polos?.nome||'').localeCompare(b.polos?.nome||'') || a.numero-b.numero);

  const coordsPorPolo = {};
  d.coords.forEach(c=>{ (coordsPorPolo[c.polo_id]=coordsPorPolo[c.polo_id]||[]).push(c); });

  const blocoCoord = !ehAdmin ? '' : `
    <div class="adminTopo" style="margin:30px 0 14px">
      <h3 style="font-size:18px">Coordenadores de polo</h3>
    </div>
    <p class="sub" style="font-size:13.5px;margin-bottom:14px">Coordenador gerencia os clubes do polo dele: cancela e transfere alunos, define mentor. Não cadastra escolas nem outros coordenadores.</p>
    <div class="tabelaWrap"><table style="min-width:640px">
      <thead><tr><th>Polo</th><th>Coordenadores</th><th></th></tr></thead>
      <tbody>${d.polos.map(p=>{
        const lista = coordsPorPolo[p.id]||[];
        return `<tr>
          <td><b>${esc(p.nome)}</b></td>
          <td>${lista.length?lista.map(c=>`<span class="funcChip">${esc(c.perfis?.nome||c.perfis?.email||'—')}
            <button class="xis" data-remcoord="${p.id}|${esc(c.perfis?.email||'')}" title="remover">×</button></span>`).join('')
            :'<span style="color:var(--suave2);font-size:13px">nenhum</span>'}</td>
          <td><button class="btn vazio peq" data-addcoord="${p.id}">+ Adicionar</button></td>
        </tr>`;
      }).join('')}</tbody></table></div>`;

  return `
  <p class="sub" style="font-size:13.5px;margin-bottom:18px">Cada clube é uma turma de ${CLUBE}. Tirar um aluno devolve a vaga; se a turma já tinha fechado, ela volta a formar e a votação recomeça. Clubes que já passaram para o alinhamento não voltam atrás.</p>

  ${turmas.map(t=>{
    const membros = (porTurma[t.id]||[]).filter(i=>i.status!=='cancelada');
    const saidos  = (porTurma[t.id]||[]).filter(i=>i.status==='cancelada');
    const aberto = clubeAberto===t.id;
    const F = FASES[t.fase];
    const destinos = d.turmas.filter(x=>x.fase==='formando' && x.ocupadas<x.capacidade && x.id!==t.id);
    const mentoresDisp = d.mentores.filter(m=>m.ativo);
    return `<div class="clubeCard ${aberto?'aberto':''}">
      <div class="clubeTopo" data-abrirclube="${t.id}">
        <div>
          <b>${esc(t.polos?.nome||'')} · Turma ${nn(t.numero)}</b>
          <span>${esc(t.slots?.rotulo||'')} · ${t.mentores?esc(t.mentores.nome):'sem mentor'}</span>
        </div>
        <div style="display:flex;align-items:center;gap:12px">
          <div class="assentosMini">${Array.from({length:t.capacidade},(_,k)=>`<i class="${k<t.ocupadas?'on':''}"></i>`).join('')}</div>
          <span class="statusPill ${F.cls}">${F.rot}</span>
          <span class="seta">${aberto?'▴':'▾'}</span>
        </div>
      </div>
      ${aberto?`<div class="clubeCorpo">
        <table class="tblMembros"><tbody>
          ${membros.length?membros.map(i=>`<tr>
            <td><b>${esc(i.aluno_nome)}</b><br><span style="color:var(--suave2);font-size:12px">${esc(i.aluno_serie)} · ${esc(i.protocolo)}</span></td>
            <td><span class="statusPill ${STATUS[i.status].cls}">${STATUS[i.status].rot}</span></td>
            <td style="text-align:right;white-space:nowrap">
              ${destinos.length?`<select class="miniSel" data-mover="${i.id}">
                <option value="">Mover para…</option>
                ${destinos.map(x=>`<option value="${x.id}">${esc(x.polos?.nome||'')} · T${nn(x.numero)} (${x.ocupadas}/${x.capacidade})</option>`).join('')}
              </select>`:''}
              <button class="btn vazio peq" data-cancelar="${i.id}|${esc(i.aluno_nome)}">Tirar</button>
            </td></tr>`).join('')
          :'<tr><td colspan="3"><div class="vazio-msg" style="border:none;padding:20px">Nenhum aluno nesta turma.</div></td></tr>'}
          ${saidos.map(i=>`<tr class="saiu">
            <td><b>${esc(i.aluno_nome)}</b><br><span style="color:var(--suave2);font-size:12px">saiu · ${esc(i.protocolo)}</span></td>
            <td><span class="statusPill st-cancelada">Cancelada</span></td>
            <td style="text-align:right"><button class="btn vazio peq" data-reativar="${i.id}">Trazer de volta</button></td>
          </tr>`).join('')}
        </tbody></table>

        <div class="clubeAcoes">
          <div>
            <label>Professor mentor</label>
            <select class="miniSel" data-mentorturma="${t.id}" ${t.fase==='formando'?'disabled':''}>
              <option value="">${t.fase==='formando'?'definido quando a turma fechar':'Escolher…'}</option>
              ${mentoresDisp.map(m=>`<option value="${m.id}" ${t.mentores&&t.mentores.nome===m.nome?'selected':''}>${esc(m.nome)} · ${esc(m.area||'')}</option>`).join('')}
            </select>
          </div>
          <p class="notaMod" style="margin:0">${t.fase==='formando'
            ? 'Enquanto forma, quem escolhe o mentor é a equipe, quando fechar em ' + t.capacidade + '.'
            : 'Definir aqui passa por cima da votação e leva a turma direto ao alinhamento.'}</p>
        </div>
      </div>`:''}
    </div>`;
  }).join('') || '<div class="vazio-msg">Nenhuma turma cadastrada.</div>'}

  ${blocoCoord}`;
}

function ligarClubes(d){
  document.querySelectorAll('[data-abrirclube]').forEach(b=>b.onclick=()=>{
    clubeAberto = clubeAberto===b.dataset.abrirclube ? null : b.dataset.abrirclube;
    render();
  });

  document.querySelectorAll('[data-cancelar]').forEach(b=>b.onclick=async()=>{
    const [id,nome]=b.dataset.cancelar.split('|');
    const motivo = prompt(`Tirar ${nome} da turma. Motivo (opcional):`);
    if(motivo===null) return;
    b.disabled=true;
    const { data, error } = await sb.rpc('fn_gestao_cancelar',{p_inscricao:id, p_motivo:motivo||null});
    if(error){ toast(erroSupabase(error), true); b.disabled=false; return; }
    toast(`${nome}: ${data}`); render();
  });

  document.querySelectorAll('[data-reativar]').forEach(b=>b.onclick=async()=>{
    b.disabled=true;
    const { error } = await sb.rpc('fn_gestao_reativar',{p_inscricao:b.dataset.reativar});
    if(error){ toast(erroSupabase(error), true); b.disabled=false; return; }
    toast('Aluno de volta na turma'); render();
  });

  document.querySelectorAll('[data-mover]').forEach(sel=>sel.onchange=async()=>{
    if(!sel.value) return;
    const { data, error } = await sb.rpc('fn_gestao_mover',{p_inscricao:sel.dataset.mover, p_destino:sel.value});
    if(error){ toast(erroSupabase(error), true); render(); return; }
    toast(data); render();
  });

  document.querySelectorAll('[data-mentorturma]').forEach(sel=>sel.onchange=async()=>{
    if(!sel.value) return;
    const { data, error } = await sb.rpc('fn_gestao_definir_mentor',{p_turma:sel.dataset.mentorturma, p_mentor:sel.value});
    if(error){ toast(erroSupabase(error), true); render(); return; }
    toast('Mentor: '+data); render();
  });

  document.querySelectorAll('[data-addcoord]').forEach(b=>b.onclick=async()=>{
    const email = prompt('E-mail de quem vai coordenar este polo.\nA pessoa precisa ter entrado no site pelo menos uma vez.');
    if(!email) return;
    const { error } = await sb.rpc('fn_admin_add_coordenador',{p_email:email.trim(), p_polo:b.dataset.addcoord});
    if(error){ toast(erroSupabase(error), true); return; }
    toast('Coordenador adicionado'); render();
  });

  document.querySelectorAll('[data-remcoord]').forEach(b=>b.onclick=async()=>{
    const [polo,email]=b.dataset.remcoord.split('|');
    if(!confirm(`Remover ${email} da coordenação deste polo?`)) return;
    const { error } = await sb.rpc('fn_admin_remove_coordenador',{p_email:email, p_polo:polo});
    if(error){ toast(erroSupabase(error), true); return; }
...
function ligarAdmin(d){
  ...
  if(adminAba==='clubes') ligarClubes(d);
