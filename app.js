...
// Botões de troca, só aparecem para quem tem mais de um papel
function seletorArea(){
  if(sessao.areas.length < 2) return '';
  return `<div class="trocaArea">
    ${sessao.areas.map(a=>`<button class="trocaBtn ${sessao.area===a?'on':''}" data-area="${a}">
      <b>${AREAS[a].rot}</b><span>${AREAS[a].sub}</span></button>`).join('')}
  </div>`;
}
