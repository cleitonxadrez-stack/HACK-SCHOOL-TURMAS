...
/* ---------- sessão ---------- */
let sessao = { user:null, perfil:null, mentor:null };

async function carregarSessao(){
  const { data:{ session } } = await sb.auth.getSession();
  sessao.user = session?.user || null;
  sessao.perfil = null; sessao.mentor = null;
  if(!sessao.user) return;

  await sb.rpc('fn_vincular_inscricoes').catch(()=>{});

  const { data:perfil } = await sb.from('perfis')
    .select('id,papel,nome,email').eq('id', sessao.user.id).maybeSingle();
  sessao.perfil = perfil;

  if(perfil?.papel === 'mentor'){
    const { data:m } = await sb.from('mentores')
      .select('*').eq('perfil_id', sessao.user.id).maybeSingle();
    sessao.mentor = m;
  }
}
  ...
  <section class="hero"><div class="molde heroGrid">
    <div>
      ...
      <div class="heroCta">
        ...
        <button class="btn vazio" data-ir="#/entrar">Já me inscrevi</button>
      </div>
      <div class="fatos">
...
function vPronto(){
  ...
  return `<section class="secao" style="border-top:none;padding-top:56px"><div class="molde"><div class="form-wrap" style="text-align:center">
    ...
    <p class="sub" style="margin:0 auto 24px">Nossa equipe entra em contato pelo WhatsApp <b style="color:var(--tinta)">${esc(recibo.zap)}</b> em até 2 dias úteis. Para acompanhar, entre com o e-mail <b style="color:var(--tinta)">${esc(recibo.email)}</b>.</p>
    <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
      <button class="btn amarelo" data-ir="#/entrar">Acessar área do aluno</button>
...
/* =====================================================================
   ENTRAR — magic link, um login para todos os papéis
   ===================================================================== */
let emailEnviado = null;

function vEntrar(){
  if(emailEnviado) return `<section class="secao" style="border-top:none"><div class="molde"><div class="loginBox">
    <div style="text-align:center"><div style="font-size:38px">✉️</div>
    <h3 style="margin-top:10px;font-size:21px">Link enviado</h3>
    <p class="sub" style="font-size:14px;margin-top:10px">Enviamos um link de acesso para <b style="color:var(--tinta)">${esc(emailEnviado)}</b>. Abra o e-mail neste mesmo aparelho e clique no link — não precisa de senha.</p>
    <p class="notaMod" style="margin-top:16px">Não chegou? Verifique o spam ou <button class="btn vazio peq" id="btnOutroEmail" style="margin-top:8px">tentar outro e-mail</button></p></div>
  </div></div></section>`;

  return `<section class="secao" style="border-top:none"><div class="molde"><div class="loginBox">
    <div style="text-align:center"><div class="olho">Área do aluno</div>
    <h3 style="margin-top:8px;font-size:22px">Entrar</h3>
    <p class="sub" style="font-size:13.5px;margin-top:8px">Use o mesmo e-mail que você informou na inscrição. Enviamos um link — sem senha para decorar.</p></div>
    <div class="campo"><label for="l_mail">Seu e-mail</label>
      <input id="l_mail" type="email" placeholder="voce@email.com" autocomplete="email"></div>
    <button class="btn amarelo" id="btnEntrar">Receber link de acesso</button>
    <p class="notaMod" style="text-align:center">Professores mentores e equipe também entram por aqui.</p>
  </div></div></section>`;
}
...
function vPainel(lista){
  if(!lista.length) return `<section class="secao" style="border-top:none;padding-top:46px"><div class="molde"><div class="form-wrap">
    <h2>Nenhuma inscrição neste e-mail</h2>
...
async function render(){
  ...
  try{
    if(rota==='#/'){
      ...
      } else if(papel==='mentor' && sessao.mentor){
...
function atualizarMenu(){
  ...
  b.textContent = sessao.user ? 'Minha área' : 'Área do aluno';
}
...
function ligarEntrar(){
  const o=$('#btnOutroEmail'); if(o) o.onclick=()=>{ emailEnviado=null; render(); };
  const b=$('#btnEntrar'); if(!b) return;
  const enviar=async()=>{
    const email=$('#l_mail').value.trim();
    if(!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)){ toast('E-mail inválido',true); return; }
    b.disabled=true; b.textContent='Enviando…';
    const { error } = await sb.auth.signInWithOtp({ email, options:{ emailRedirectTo: location.origin + location.pathname } });
    b.disabled=false; b.textContent='Receber link de acesso';
    if(error){ toast(erroSupabase(error), true); return; }
    emailEnviado=email; render();
  };
  b.onclick=enviar;
  $('#l_mail').onkeydown=e=>{ if(e.key==='Enter') enviar(); };
}
